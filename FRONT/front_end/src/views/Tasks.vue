<template>
    <div class="bg-gray-100 min-h-screen">
      <Header 
        title="NOVA LEAD" 
        primaryActionText="Nouvelle tâche" 
        @primaryAction="showTaskModal = true" 
          ...
          <table class="min-w-full bg-white">
            ...
            <tbody>
              <tr v-for="task in filteredTasks" :key="task._id">
                ...
                <td class="border px-4 py-2 text-center">
              <span v-if="task.statut === 'en attente'" class="inline-block w-4 h-4 bg-yellow-400 rounded-full"></span>
              <span v-else-if="task.statut === 'en cours'" class="inline-block w-4 h-4 bg-orange-500 rounded-full"></span>
              <span v-else-if="task.statut === 'cloturé'" class="inline-block w-4 h-4 bg-green-500 rounded-full"></span>
            ...
            <td class="border px-4 py-2">
              {{ Array.isArray(task.assignedAgents) ? task.assignedAgents.length : 0 }} agent(s)
                ...
                <div class="relative inline-block text-left">
                  ...
                  <button @click="toggleDropdown(task._id)" class="text-gray-700 hover:text-gray-500 focus:outline-none">
                ...
                <div v-if="dropdownOpen === task._id" class="absolute right-0 mt-2 w-28 bg-white border border-gray-200 rounded shadow-lg" style="position: fixed; z-index: 9999;">
                  <button @click="openTaskDetails(task)" class="flex items-center px-4 py-2 text-gray-700 hover:bg-gray-100 w-full text-left">
              ...
              <button  @click="editTask(task)" class="flex items-center px-4 py-2 text-gray-700 hover:bg-gray-100 w-full text-left">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <button v-if="task.statut !== 'cloturé' " @click="closeTask(task)" class="flex items-center px-4 py-2 text-gray-700 hover:bg-gray-100 w-full text-left">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div v-if="showCloseTaskModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
        <div class="bg-white p-6 rounded shadow-lg">
          ...
          <div class="flex justify-center space-x-4 text-center">
            ...
            <button @click="confirmCloseTask" class="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600">Clôturer</button>
          </div>
        </div>
      </div>
